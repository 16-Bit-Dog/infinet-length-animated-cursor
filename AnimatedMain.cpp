...
BOOL WINAPI Handler(DWORD endType) {
...
void revertCursor(DWORD cursorID) {
	HCURSOR cursor = LoadCursorFromFileA("C:\\Windows\\Cursors\\wait_im.cur"); // place holder if I really messed up
...
void ImageToCursor(){
	...
	std::cout << "\nEnter the cursor type to modify <-- if you enter a invalid one the program will not work:";
	std::cin >> cursorType;
	std::cout << "\n";
	...
	std::cout << "enter the delay until each cursor picture changes (this value is miliseconds) --> I reccomend making this the fps of the video (or image compilation); math would be 'INPUT = 1000/fpsOfVideo' --> ex: if video is 24fps (like youtube),  INPUT = 1000/24, INPUT = 42  --> also as a side note; your cursor pointer is pointing at the very left and middle of the square\n";
...
void VideoToCursor() {
	...
	std::cout << "NOTE 1: once you do this operation, you can use them again with method y/Y (you need to look at the largest image name minus 1 to get the number of images). This method also may take more time depending on your computers speed (cpu and storage). ONE MORE THING, ALSO, if you previously did a video, either delete the resulting files or use the image version of this program (faster and will not use up storage space)\n\nNOTE; restarting your computer resets any changed cursors (you can also use method 'r'/'R'). You also need ffmpeg.exe to use this feature of the program; download it from their official website and when extracting the archive it should be in ffmpeg/bin/'ffmpeg' \n\nto use the program(multipule can be open at once) :\n1.fill a folder with the video you want to be rotated through(name must be 'video.format' -make sure the folder does not have spaces (and name is just lowercase letters - video and folder)   \n2.drag and drop the folder in this window\n3.click enter (sometimes you need to do it twice) \n4.chose which cursor you want to animate(ex: hand arrow) - and number of images to cycle through(putting a impossible value will result in a crash)\n5.the cursor will animate\n\n\n DRAG AND DROP THE FOLDER HERE!!!, click enter\n\n";
	...
	std::cout << "drag and drop the ffmpeg program (the .exe file) into this command line and click enter (should be called ffmpeg.exe)\n\n" << std::endl;
...
int main() { // yes a switch case is better for this, but I really dont care, this is not active runtime check for 99.999% of the program
	...
	std::cout << "click 'Y' or 'y' and enter for 'image to cursor', any other letter turns on 'video to cursor' mode\n\n\n\n\n";
	std::cin >> check;
