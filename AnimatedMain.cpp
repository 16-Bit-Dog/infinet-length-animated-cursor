...
: end loop
    ...
    void createRenderPass() {
        ...
        */
        ...
        uboLayoutBinding.descriptorCount = 1; //how many descriptors are used; could be used like an array to control bones of model
...
*/
    ...
    void createTextureImage() { // get and make texture...
        ...
        for (int i = 0; i < textureCount; i++) {
            ...
            if (i < textureCount) {
                int positionOfNum = TEXTURE_PATH.find(std::to_string(i + 1));
                TEXTURE_PATH.replace(positionOfNum, size_t(1), std::to_string(i + 2)); // limits what numbers I can use, but I'm find because I dont care right now; not feeling like converting more of my models to obj's
            }
        }
    ...
    void loadModel() {
        ...
        for (int loop = 0; loop <= modelCount; loop++) {
            ...
            if (loop <= modelCount - 2) {
                positionOfNum = MODEL_PATH.find(std::to_string(loop + 1));
                MODEL_PATH.replace(positionOfNum, size_t(1), std::to_string(loop + 2)); // limits what numbers I can use, but I'm find because I dont care right now; not feeling like converting more of my models to obj's
            }
    ...
    void createDescriptorPool() {
        ...
        for (int i = 0; i < ((textureCount-1) * 2)-1; i++) {
...
*/
        ...
        for (size_t i = 0; i < commandBuffers.size(); i++) { // for all command buffers do the following:
            ...
            */
            ...
            for (int y = 0; y < vertexBuffer.size(); y++) { 
               ...
               // offsets[0] = { static_cast<unsigned long long> (5) * y }; // make empty offsets array
 ...
 // but I need to redo the pipeline, raise offset and reduce extent
    ...
    void updateUniformBuffer(uint32_t currentImage) { // change model stuff here and views here... 
        ...
        ubo.model = glm::rotate(glm::mat4(1.0f), glm::radians(1.0f*5 + time*100), glm::vec3(0.0f, 0.0f, 1.0f)); //model power to rotate, angle to rotate, angle to rotate (x,y,z) rotate 
